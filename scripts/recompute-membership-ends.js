...
try {
  // Delegate to the CJS script which uses the repo-root DB path
  execSync('node scripts/recompute-membership-ends.cjs', { stdio: 'inherit' });
} catch (e) {
  console.error('Failed to run recompute script:', e && e.message);
  process.exit(1);
}
